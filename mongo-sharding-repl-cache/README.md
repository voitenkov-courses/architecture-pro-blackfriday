...
Для инициализации шардирования и репликации, а также заполнения MongoDB данными выполните следующий скрипт:
...
2. Инициализирует реплика-сет **shard1** (3 реплики, порты 27021, 27023, 27025)
3. Инициализирует реплика-сет **shard2** (3 реплики, порты 27022, 27024, 27026)
4. Инициализируем роутер **mongos_router** (порт 27017), добавляем **shard1** (порт primary 27021) и **shard1** (порт primary 27022)
5. Наполняет БД тестовыми данными

### Настройка кластера Redis

Для настройки кластера Redis подключитесь к контейнеру инстанса Redis:
```
docker-compose exec -it redis_1 sh
```
и запустите следующую команду:
```
echo "yes" | redis-cli --cluster create 173.17.0.2:6379 173.17.0.3:6379 173.17.0.4:6379 173.17.0.5:6379 173.17.0.6:6379 173.17.0.7:6379 173.17.0.8:6379 173.17.0.9:6379 173.17.0.10:6379 --cluster-replicas 2
```
Не забудьте выйти из контейнера, набрав команду `exit`.
